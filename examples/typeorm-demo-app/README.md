...
- Creates the demo database if needed with `typeorm:setup:example`
- Runs checked-in TypeORM migrations explicitly with `typeorm:migrate:example`
...
- `typeorm:setup:example` prepares the Postgres database
- `typeorm:migrate:example` runs the checked-in TypeORM migrations
